      ...
      <div className='start'>

        {
          launch && <div className='mission-briefing-container'>
            {launch && <div className='planet-name-text'>
              Planet: {PLANETS[planet].name}</div>
            }
            {
              launch && <div className='planet-name-text'>
                Hazard Lvl: {PLANETS[planet].hazardLevel}</div>
            }
            {
              launch && <div className='planet-name-text'>
                Distance: {PLANETS[planet].distance} Light Years</div>
            }
            {
              launch && <div className='mission-name-text'>
                {PLANETS[planet].description}</div>
            }
            {
              launch && <div className='launch-btn'
                onClick={startLaunch}>LAUNCH</div>
            }
          </div>}
        {
          missionover && <div className='skip-btn'
            onClick={skipAnimation}>Skip →</div>
        }
        <div className='mission-container'>
          {
            missionover && <div className='ship'>
              <img className="explorer-ship"
                src="images\explorer_ship.png"></img></div>
          }
          {
            missionover && <div className='planet'></div>
          }
          {
            results && <Link to="/MissionResults"
              className='mission-results-btn'>Mission Results</Link>
          }
        </div>
        <div className='hud'>
